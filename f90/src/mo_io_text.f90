...
CONTAINS
  ...
  SUBROUTINE open_out_text()
    ...
    open(unit=noutseas, file=stmp,action="write", status="replace", &
         form="formatted", recl=139*25, iostat=ierr)
    if (ierr > 0) call error_opening(isroutine, stmp)
    ...
    write(noutseas,form1) &
         ...
         "Tlshade", "disc13", "disc13_long", "disc13_a", "disc13_ab", &
         "disc13_asal", "disc13_b", "CsCa", "CiCa", "CcCa", &
         "gm", "gs", "ave_dair_13C", "isoprene%flux", "diff_par", &
         "par", "ta", "u", "ustar", "rhov", &
         "zL", "press%Pa", "relative%humidity", "CO2air","Tair40", &
         "CO2air40", "Vpd40", "Tleaf40", "Sun_A40", "Sun_gs_mol40", &
         "Sun_rbCO240", "Sun_cs40", "Sun_ci40", "Sun_cc40", "Sun_wj40", &
         "Sun_wc40", "Quantum_sun40", "Sun_rh40", "Sun_vpd40", "H_40", &
         "Trans_40", "canps_40", "Tleaf_40", "disc13_40", "disc13C_long_40", &
         "disc13C_a_40", "disc13C_ab_40", "disc13C_asl_40", "disc13C_b_40", "cica_40", &
         "gs_40", "Fisoprene_40", "PARdirect_40", "PARdiffuse_40", "NIRdirect_40", &
         "NIRdiffuse_40", "H_30", "Trans_30", "canps_30", "Tleaf_30", &
         "disc13_30", "disc13C_long_30", "disc13C_a_30", "disc13C_ab_30", "disc13C_asl_30", &
         ...
         "H_10", "Trans_10", "canps_10", "Tleaf_10", "disc13_10", &
         "disc13C_long_10", "disc13C_a_10", "disc13C_ab_10", "disc13C_asl_10", "disc13C_b_10", &
         "cica_10", "gs_10", "Fisoprene_10", "PARdirect_10", "PARdiffuse_10", &
         "NIRdirect_10", "NIRdiffuse_10", "H_1", "Trans_1", "canps_1", &
         "Tleaf_1", "disc13_1", "disc13C_long_1", "disc13C_a_1", "disc13C_ab_1", &
         "disc13C_asl_1", "disc13C_b_1", "cica_1", "gs_1", "Fisoprene_1", &
         "PARdirect_1", "PARdiffuse_1", "NIRdirect_1", "NIRdiffuse_1"
    ...
    open(unit=noutflux, file=stmp,action="write", status="replace", &
         form="formatted", recl=45*25, iostat=ierr)
    if (ierr > 0) call error_opening(isroutine, stmp)
  ...
  SUBROUTINE read_in_text()
    ...
    read(ninmet,*,iostat=ierr) dayy, hhrr, in01, in02, in03, in04, &
         in05, in06, in07, in08, in09, in10, in11, flag, in13, in14
    if (ierr > 0) call error_reading(isroutine, ninmet)
    if (ierr < 0) call error_reading(isroutine, ninmet, 'reached EOF.')
    input%dayy         = dayy
    input%hhrr         = hhrr
  ...
  SUBROUTINE write_out_text()
    ...
    write(noutseas,form1,iostat=ierr) &
         time%daytime, output%netrad, output%sumrn, &
         output%sumh, output%sumle, output%can_ps_mol, &
         output%can_gpp, output%c_transpiration_mole, output%canresp, &
         soil%respiration_mole, bole%respiration_mole, soil%gsoil, &
         output%sumksi, output%tleaf_mean, output%tavg_sun, &
         output%tavg_shd, output%ave_disc13, output%ave_disc13_long, &
         output%ave_disc13_a, output%ave_disc13_ab, output%ave_disc13_asal, &
         output%ave_disc13_b, output%ave_csca, output%ave_cica, &
         output%ave_ccca, output%ave_gm, output%ave_gs, &
         output%ave_daC13, output%isoprene_efflux, output%diff_par, &
         input%parin, input%ta, input%wnd, &
         met%ustar, met%rhova_g, met%zl, &
         met%press_Pa, met%relative_humidity, input%co2air, &
         prof%tair(ncl), prof%co2_air(ncl), prof%vpd_air(ncl), &
         prof%tleaf(ncl), prof%sun_A(ncl), prof%sun_gs_mol(ncl), &
         prof%sun_rbco2(ncl), prof%sun_cs(ncl), prof%sun_ci(ncl), &
         prof%sun_cc(ncl), prof%sun_wj(ncl), prof%sun_wc(ncl), &
         solar%quantum_sun(ncl), prof%sun_rh(ncl), prof%sun_vpd(ncl), &
         prof%dHdz(ncl), prof%dLEdz(ncl,1), prof%dPsdz(ncl), &
         prof%tleaf(ncl), prof%disc13C(ncl), prof%disc13C_long(ncl), &
         prof%disc13C_a(ncl), prof%disc13C_ab(ncl), prof%disc13C_asal(ncl), &
         prof%disc13C_b(ncl), prof%cica(ncl), prof%dStomCondz(ncl), &
         prof%isopreneflux(ncl), solar%beam_flux_par(ncl), solar%par_down(ncl), &
         solar%beam_flux_nir(ncl), solar%nir_dn(ncl), prof%dHdz(3*ncl/4), &
         prof%dLEdz(3*ncl/4,1), prof%dPsdz(3*ncl/4), prof%tleaf(3*ncl/4), &
         prof%disc13C(3*ncl/4), prof%disc13C_long(3*ncl/4), prof%disc13C_a(3*ncl/4), &
         prof%disc13C_ab(3*ncl/4), prof%disc13C_asal(3*ncl/4), prof%disc13C_b(3*ncl/4), &
         prof%cica(3*ncl/4), prof%dStomCondz(3*ncl/4), prof%isopreneflux(3*ncl/4), &
         solar%beam_flux_par(3*ncl/4), solar%par_down(3*ncl/4), solar%beam_flux_nir(3*ncl/4), &
         solar%nir_dn(3*ncl/4), prof%dHdz(ncl/2), prof%dLEdz(ncl/2,1), &
         prof%dPsdz(ncl/2), prof%tleaf(ncl/2), prof%disc13C(ncl/2), &
         prof%disc13C_long(ncl/2), prof%disc13C_a(ncl/2), prof%disc13C_ab(ncl/2), &
         prof%disc13C_asal(ncl/2), prof%disc13C_b(ncl/2), prof%cica(ncl/2), &
         prof%dStomCondz(ncl/2), prof%isopreneflux(ncl/2), solar%beam_flux_par(ncl/2), &
         solar%par_down(ncl/2), solar%beam_flux_nir(ncl/2), solar%nir_dn(ncl/2), &
         prof%dHdz(ncl/4), prof%dLEdz(ncl/4,1), prof%dPsdz(ncl/4), &
         prof%tleaf(ncl/4), prof%disc13C(ncl/4), prof%disc13C_long(ncl/4), &
         prof%disc13C_a(ncl/4), prof%disc13C_ab(ncl/4), prof%disc13C_asal(ncl/4), &
         prof%disc13C_b(ncl/4), prof%cica(ncl/4), prof%dStomCondz(ncl/4), &
         prof%isopreneflux(ncl/4), solar%beam_flux_par(ncl/4), solar%par_down(ncl/4), &
         solar%beam_flux_nir(ncl/4), solar%nir_dn(ncl/4), prof%dHdz(1), &
         prof%dLEdz(1,1), prof%dPsdz(1), prof%tleaf(1), &
         prof%disc13C(1), prof%disc13C_long(1), prof%disc13C_a(1), &
         prof%disc13C_ab(1), prof%disc13C_asal(1), prof%disc13C_b(1), &
         prof%cica(1), prof%dStomCondz(1), prof%isopreneflux(1), &
  ...
  SUBROUTINE write_prof_text()
    ...
    do j=1, ncl
       write(noutflux,form1,iostat=ierr) &
            ...
            prof%dStomCondz(j)*1e3_wp, prof%dStomCondz_sun(j)*1e3_wp, &
            prof%dStomCondz_shd(j)*1e3_wp, prof%disc13C_long(j), &
